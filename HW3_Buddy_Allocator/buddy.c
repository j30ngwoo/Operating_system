...
int bit_ishalfset(char* array, int index)
{
  index = index >> 1;
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    if (bit_ishalfset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
int
bd_initfree_pair(int k, int bi, int l) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_ishalfset(bd_sizes[k].alloc, bi)) {
    free = BLK_SIZE(k);
    if(bi != l)
...
bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    ...
    free += bd_initfree_pair(k, left, left);
    if(right <= left)
      continue;
    free += bd_initfree_pair(k, right, left);
  }
  return free;
}
